...
// const PaymentDetails = ({ totals }) => {
//   return (
//   <div className="mt-10 px-8 pt-8 lg:mt-0 rounded-3xl border border-[#E4D7D1] dark:border-gray-800 bg-white dark:bg-secondary-dark-bg shadow-sm">
...
//       <div className="flex flex-col sm:flex-row  sm:gap-2 lg:gap-4">
...
//           <p className="font-semibold text-gray-900">${(totals?.discountTotal ?? 0).toFixed(2)}</p>
//         </div>
//         <div className="flex items-center justify-between">
...
//         <p className="text-2xl font-semibold text-gray-900"> ${((totals?.discountTotal ?? 0) + 8.0).toFixed(2)}</p>
//       </div>
//     </div>
...
//   )
// }
//src/front/checout/components/PaymentDetails.jsx
import React from 'react'
import { useCart } from '../../contexts/CartContext';
const PaymentDetails = () => {
   const { cart, totals } = useCart();
    return (
        <div className=" text-white p-8 rounded-3xl border border-brand-cream/50 dark:border-gray-100 shadow-sm">
            <h3 className="font-serif text-2xl mb-6 text-brand-burgundy font-semibold dark:text-brand-cream">Secure Payment</h3>
            <div className="space-y-4">
                <div className="flex justify-between border-b border-gray/10 dark:border-gray-100 pb-4 text-gray-500">
                    <span className="opacity-80 text-black dark:text-gray-100">Subtotal</span>
                    <span className='dark:text-brand-cream'>${totals?.finalTotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between border-b border-brand-cream dark:border-gray-100 pb-4 text-gray-500">
                    <span className="opacity-80 text-black dark:text-gray-100">Saved</span>
                    <span className='dark:text-brand-cream'>${(totals?.originalTotal-totals?.finalTotal).toFixed(2)}</span>
                </div>
                <div className="flex justify-between border-b border-brand-cream dark:border-gray-100 pb-4 text-gray-500">
                    <span className="opacity-80 text-black dark:text-gray-100">Shipping (Standard)</span>
                    <span className="font-medium dark:text-brand-cream">$5.00</span>
                </div>
                <div className="flex justify-between text-2xl font-bold pt-4 text-gray-600 dark:text-brand-cream">
                    <span >Total Amount</span>
                    <span>${totals?.finalTotalIncludingShipment.toFixed(2)}</span>
                </div>
            </div>
            
            <p className="text-[10px] mt-6 opacity-60 text-center uppercase tracking-widest text-gray-400">
